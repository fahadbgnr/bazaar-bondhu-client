...
const AdvertisementCarousel = () => {
  ...
  if (ads.length === 0) {
    return (
      <div className="max-w-7xl mx-auto p-6 text-center text-gray-500 italic">
      ...
      >
        {ads.map((ad, index) => (
          <SwiperSlide key={ad._id?.$oid || ad._id || index}>
            <div className="rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-500 bg-white">
              {ad.image ? (
                <img
                  src={ad.image}
                ...
                <div className="w-full h-56 sm:h-64 md:h-72 bg-gray-200 flex items-center justify-center text-gray-500">
                  No Image Available
                </div>
              )}
              <div className="p-4">
                <h3 className="text-lg font-semibold truncate">{ad.title || 'Untitled'}</h3>
